...
import MovieListSlider from "../movie-list/movie-list-slider";
...
const PeoplePage = ({
  ...
  let modifiedBiography = [];

  if (biography) {
    modifiedBiography = biography.split("\n\n").map((text, i) => (
      <p key={i} className="people-page__biography">
        {text}
      </p>
    ));
  }
  ...
  return (
    <>
      <section className="people-page">
        <Container>
          <div className="people-page__preview">
            <div className="people-page__poster-container">
              <img
                className="people-page__poster"
                src={
                  profile_path
            ...
            <div className="people-page__text-content">
              <h2 className="people-page__name">{name}</h2>
              <a
                className="people-page__link-imdb"
              ...
              {modifiedBiography.length ? (
                <h5 className="people-page__biography-title">Biography</h5>
              ) : null}
              {modifiedBiography}
            </div>
          </div>
          {cast.length ? (
            <MovieListSlider
              title={"Known For"}
              moviesArr={cast.filter((item) => {
                if (item.release_date) {
                  const { release_date } = item;
                  const a = Date.parse(release_date);
                  if (a - Date.now() < 0) {
                    return item;
                  }
                }
...
const mapStateToProps = ({ person: { personDetails, loading, error } }) => {
  return {
    personDetails,
    loading,
    error,
...
const mapDispatchToProps = (dispatch, { movieService }) => {
  return {
    fetchPersonDetails: (actorId) => {
      return fetchPersonDetails(dispatch, movieService)(actorId);
    },
  };
};
